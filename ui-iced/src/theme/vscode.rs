...
pub fn parse_vscode_theme(json: &str) -> Option<SemanticColors> {
    ...
    if let Some(ref color_map) = theme_json.colors {
        // Editor
        if let Some(c) = color_map.get("editor.background").and_then(|s| parse_hex_color(s)) {
            colors.editor_background = c;
            colors.background = c;
            colors.tab_active_background = c;
        }
        if let Some(c) = color_map.get("editor.foreground").and_then(|s| parse_hex_color(s)) {
        ...
        // Sidebar
        if let Some(c) = color_map.get("sideBar.background").and_then(|s| parse_hex_color(s)) {
            colors.sidebar_background = c;
        }
        if let Some(c) = color_map.get("sideBar.foreground").and_then(|s| parse_hex_color(s)) {
            colors.sidebar_foreground = c;
        }
        
        // Activity bar
        if let Some(c) = color_map.get("activityBar.background").and_then(|s| parse_hex_color(s)) {
            colors.activitybar_background = c;
        }
        if let Some(c) = color_map.get("activityBar.foreground").and_then(|s| parse_hex_color(s)) {
            colors.activitybar_foreground = c;
        }
        
        // Status bar
        if let Some(c) = color_map.get("statusBar.background").and_then(|s| parse_hex_color(s)) {
            colors.statusbar_background = c;
        }
        if let Some(c) = color_map.get("statusBar.foreground").and_then(|s| parse_hex_color(s)) {
            colors.statusbar_foreground = c;
        }
        
        // Tabs
        if let Some(c) = color_map.get("tab.inactiveBackground").and_then(|s| parse_hex_color(s)) {
            colors.tab_background = c;
        }
        if let Some(c) = color_map.get("tab.activeBackground").and_then(|s| parse_hex_color(s)) {
            colors.tab_active_background = c;
        }
        if let Some(c) = color_map.get("tab.hoverBackground").and_then(|s| parse_hex_color(s)) {
            colors.tab_hover_background = c;
        }
        
        // Input
        if let Some(c) = color_map.get("input.background").and_then(|s| parse_hex_color(s)) {
            colors.input_background = c;
        }
        if let Some(c) = color_map.get("input.foreground").and_then(|s| parse_hex_color(s)) {
            colors.input_foreground = c;
        }
        if let Some(c) = color_map.get("input.border").and_then(|s| parse_hex_color(s)) {
            colors.input_border = c;
        }
        
        // Accent colors (try multiple sources)
        for key in &["button.background", "focusBorder", "list.activeSelectionBackground"] {
        ...
        // Selection
        if let Some(c) = color_map.get("editor.selectionBackground").and_then(|s| parse_hex_color(s)) {
            colors.selection = c;
        }
        
        // List hover
        if let Some(c) = color_map.get("list.hoverBackground").and_then(|s| parse_hex_color(s)) {
            colors.list_hover = c;
        }
        
        // Border
        if let Some(c) = color_map.get("panel.border").and_then(|s| parse_hex_color(s)) {
            colors.border = c;
        }
        
        // Dim foreground
        if let Some(c) = color_map.get("descriptionForeground").and_then(|s| parse_hex_color(s)) {
                ...
                if let Some(fg) = settings.get("foreground").and_then(|v| v.as_str()) {
                    if let Some(color) = parse_hex_color(fg) {
                        let scope_str = match scope {
                            ...
                            VSCodeScope::Array(arr) => arr.first().map(|s| s.as_str()).unwrap_or(""),
                        };
                        ...
                        } else if scope_str.contains("entity.name.type") || scope_str.contains("support.type") {
